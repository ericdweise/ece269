...
def build_d2_plots(N, m_values, s_values, noise_var):
    ...
    # save data
    np.save(f'./data/part-d2-N-{N}-sigma-{noise_var}-m-values', m_coords)
    np.save(f'./data/part-d2-N-{N}-sigma-{noise_var}-s-values', s_coords)
    np.save(f'./data/part-d2-N-{N}-sigma-{noise_var}-esr-values', esr_val)
    np.save(f'./data/part-d2-N-{N}-sigma-{noise_var}-err-values', err_val)

    # Plot Noiseless Phase Transition
    plot_esr(f'd2', f'{N}-sigma-{noise_var}', m_coords, s_coords, esr_val)

    # Plot Error values
    plot_error(f'd2', f'{N}-sigma-{noise_var}', m_coords, s_coords, err_val)
